...
// Upsert baris session (dipakai endpoint sync extension Chrome)
export async function upsertVibesSession({ key, session_value, project_id = null, username = null, account_status = null, source = 'chrome-extension', valid = null }) {
  const pool = getPool();
  await pool.query(
    `INSERT INTO vibes_sessions (key, session_value, project_id, username, account_status, source, valid, captured_at, synced_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
     ON CONFLICT (key) DO UPDATE SET
       session_value = EXCLUDED.session_value,
       project_id = COALESCE(EXCLUDED.project_id, vibes_sessions.project_id),
       username = EXCLUDED.username,
       account_status = EXCLUDED.account_status,
       source = EXCLUDED.source,
       valid = EXCLUDED.valid,
       synced_at = NOW()`,
    [key, session_value, project_id, username, account_status, source, valid]);
}
...
export async function generateVibesVideo({ prompt, aspectRatio, imageUrl, imageEntId, contentItemId, imageBase64, imageMimeType, ...opts }) {
  if (!prompt || !String(prompt).trim()) {
    throw Object.assign(new Error('Prompt wajib diisi'), { status: 400 });
  }
  const o = { ...DEFAULTS, ...opts, aspectRatio: aspectRatio || DEFAULTS.aspectRatio };
  if (!ALLOWED_RESOLUTIONS.includes(o.resolution)) {
    throw Object.assign(new Error('Resolusi hanya 480p atau 720p'), { status: 400 });
  }
  const allowedRatios = ['auto', '9:16', '16:9'];
  if (o.aspectRatio && !allowedRatios.includes(o.aspectRatio)) {
    throw Object.assign(new Error('Rasio hanya auto, 9:16, atau 16:9'), { status: 400 });
  }
  const rows = await listVibesAccounts();
  if (!rows.length) {
...
async function generateWithAccount(account, promptText, o, media) {
  ...
  if (imageBase64 && (!finalImageEntId || !finalImageUrl)) {
    ...
    // Rasio video ikut rasio ASLI gambar (dideteksi server saat upload)
    detectedAspectRatio = uploadData.aspectRatio || null;
  }
  ...
  const config = {
    ...
    // Rasio: pilihan user; 'auto' = i2v ikut rasio gambar asli, t2v 9:16
    aspectRatio: o.aspectRatio === 'auto'
      ? ((isI2V && detectedAspectRatio) ? detectedAspectRatio : DEFAULTS.aspectRatio)
      : o.aspectRatio,
