...
After confirmed payment, the server generates a new random password and a six-digit, one-time login OTP. Both are emailed to the order address together with the new expiry. The OTP expires after 15 minutes and is consumed on its first successful login. For an existing account, the new password replaces the previous password and invalidates old dashboard sessions only after the access email has been accepted by SMTP. Existing passwords cannot be emailed because only salted password verifiers are stored.

Checkout status is protected by a random HttpOnly, Secure, SameSite cookie whose SHA-256 hash is stored with the order. Raw passwords and OTPs are never placed in `billing.json`, API responses, cookies, or logs. If email delivery fails, the paid order remains active with delivery marked pending and the background verifier retries it; the account's current password is not changed by a failed email attempt. The browser shows a payment-success thank-you screen and directs the buyer to check Inbox or Spam, rather than logging in automatically.
