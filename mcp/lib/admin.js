...
const publicUser=u=>({id:u.id,email:u.email,label:u.label,active:u.active,expiresAt:u.expiresAt,maxSessions:u.maxSessions,rpmLimit:normalizeRpm(u.rpmLimit),keyId:u.keyId??null,createdAt:u.createdAt,updatedAt:u.updatedAt});
...
export class UserStore{
  ...
  async upsert(input){const email=normalizeEmail(input.email),stamp=new Date(this.now()).toISOString();let user=this.findByEmail(email),created=!user,initialPassword,before=user&&{...user};
    if(!user){initialPassword=password();user={id:randomUUID(),email,label:input.label??'',active:input.active??true,expiresAt:input.expiresAt??null,maxSessions:input.maxSessions??1,rpmLimit:input.rpmLimit??20,keyId:null,...verifier(initialPassword),passwordVersion:1,passwordUpdatedAt:stamp,createdAt:stamp,updatedAt:stamp};this.records.push(user)}
    else {if(input.label!==undefined)user.label=input.label;if(input.maxSessions!==undefined)user.maxSessions=input.maxSessions;if(input.rpmLimit!==undefined)user.rpmLimit=input.rpmLimit;if(input.expiresAt!==undefined)user.expiresAt=input.expiresAt;if(input.active!==undefined)user.active=input.active;user.updatedAt=stamp}
...
export function validUserInput(v,{partial=false}={}){if(!v||typeof v!=='object'||Array.isArray(v))return false;const allowed=partial?['label','active','expiresAt','maxSessions','rpmLimit']:['email','label','active','expiresAt','maxSessions','rpmLimit'];if(Object.keys(v).some(k=>!allowed.includes(k)))return false;if(!partial&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizeEmail(v.email)))return false;if(v.label!==undefined&&(typeof v.label!=='string'||v.label.length>200))return false;if(v.active!==undefined&&typeof v.active!=='boolean')return false;if(v.maxSessions!==undefined&&(!Number.isInteger(v.maxSessions)||v.maxSessions<1||v.maxSessions>1000))return false;if(v.rpmLimit!==undefined&&(!Number.isInteger(v.rpmLimit)||v.rpmLimit<1||v.rpmLimit>600))return false;if(v.expiresAt!==undefined&&v.expiresAt!==null&&(typeof v.expiresAt!=='string'||!Number.isFinite(Date.parse(v.expiresAt))))return false;return !partial||Object.keys(v).length>0}
function normalizeRpm(value){return Number.isInteger(value)&&value>=1&&value<=600?value:20}
export {publicUser};
