...
export class BillingService {
  ...
  async createPublicOrder({email,phone,user,passwordVerifier}){
    ...
    if(existing){await this.enqueue(async()=>{existing.checkoutTokenHash=checkoutTokenHash;existing.phone=phone;if(user)existing.replacementVerifier={...passwordVerifier};else existing.onboarding={...passwordVerifier};await this.persist()});return {order:await this.publicOrder(existing),checkoutToken}}
    const order=await this.newOrder({userId:user?.id??null,email,phone,accountAction:user?'extend':'new',checkoutTokenHash,onboarding:user?null:{...passwordVerifier},replacementVerifier:user?{...passwordVerifier}:null});return {order:await this.publicOrder(order),checkoutToken};
  }
