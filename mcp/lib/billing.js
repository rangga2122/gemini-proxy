...
export class BillingService {
  ...
  async performCredentialDelivery(order){const last=Date.parse(order.lastCredentialDeliveryAttemptAt||'');if(Number.isFinite(last)&&this.now()-last<60000)return order;const user=this.users.get(order.userId);if(!user||typeof this.mailer?.sendPaidAccess!=='function')return order;await this.enqueue(async()=>{order.credentialDeliveryStatus='pending';order.credentialDeliveryAttempts=Number(order.credentialDeliveryAttempts||0)+1;order.lastCredentialDeliveryAttemptAt=new Date(this.now()).toISOString();await this.persist()});const credentials=this.credentialGenerator(),otpExpiresAt=new Date(this.now()+15*60*1000).toISOString();try{await this.mailer.sendPaidAccess({to:user.email,password:credentials.password,otp:credentials.otp,otpExpiresMinutes:15,orderCode:order.orderCode,newExpiresAt:order.targetExpiry});await this.users.setPaidCredentials(user.id,{password:credentials.password,otp:credentials.otp,otpExpiresAt});try{await this.onCredentialsSet?.(user.id)}catch{}await this.enqueue(async()=>{order.credentialDeliveryStatus='sent';order.credentialsDeliveredAt=new Date(this.now()).toISOString();await this.persist()})}catch{return order}return order}
...
function generateCredentials(){return {password:randomBytes(24).toString('base64url'),otp:String(randomInt(0,1000000)).padStart(6,'0')}}
