...
test('dashboard proxy authenticates and forwards only the exact allowlist',async t=>{
  ...
  const stats=await (await fetch(base+'/admin/stats',{headers:{cookie}})).json();assert.deepEqual(stats.totals,{imageGenerate:1,imageEdit:2,vision:0,chat:1,audio:1,images:3,overall:5});
...
test('dashboard proxy applies the worker limit configured for the account',async t=>{
  const gen=http.createServer(async(req,res)=>{for await(const chunk of req)void chunk;await new Promise(resolve=>setTimeout(resolve,100));res.writeHead(200,{'content-type':'application/json'});res.end('{"ok":true}')});const genUrl=await listen(gen);t.after(()=>new Promise(r=>gen.close(r)));
  const app=await createApp({stateDir:await mkdtemp(join(tmpdir(),'dashboard-workers-')),genUrl,adminEmail:'admin@example.com',adminPasswordSalt:salt,adminPasswordHash:hash});const base=await listen(app.server);t.after(()=>app.close());
  let response=await fetch(base+'/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:'admin@example.com',credential:password})});const adminCookie=response.headers.get('set-cookie');response=await fetch(base+'/admin/users',{method:'POST',headers:{cookie:adminCookie,'content-type':'application/json'},body:JSON.stringify({email:'workers@example.com',rpmLimit:20,workerLimit:3})});const made=await response.json();response=await fetch(base+'/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:'workers@example.com',credential:made.password})});const userCookie=response.headers.get('set-cookie'),options={method:'POST',headers:{cookie:userCookie,'content-type':'application/json'},body:JSON.stringify({prompt:'hello'})};const results=await Promise.all(Array.from({length:4},()=>fetch(base+'/dashboard/v1/chat/completions',options)));assert.deepEqual(results.map(value=>value.status).sort(),[200,200,200,429]);assert.equal((await results.find(value=>value.status===429).json()).workerLimit,3);
});
